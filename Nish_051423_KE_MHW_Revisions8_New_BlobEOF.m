...
mask = (abs(GOA_SST_regs) > fig_sig_matrix);
mask = double(mask);
mask(mask == 0) = NaN;
...
%        set(h, 'Color', 'w');
    ...
    limit = 0.8;  %here you set the max/min value you want shaded
...
h.FontWeight = 'bold';
h.FontSize = 12;
h.Ticks = [-0.75 -0.5 -0.25 0 0.25 0.5 0.75];
h.LineWidth = 0.05;
%xlabel(h, '^{o}C')


% Plot KI & Blob EOF
subplot('position',[0.05,0.10,0.63,0.35])
...
HVD_Cop_LS_EOF_2 = HVD_Cop_LS_EOF_2(1:2, :);
...
%tiledlayout(1, 4);
%nexttile([1,3])
x = plot(KI, '--', 'Color', '#0072BD', 'LineWidth',1.4);
yline(0,'--', 'LineWidth',1.5);
xlim([0 38]);
ylim([-3 3]);
hold on
a = plot(GOA_PC2, '--', 'Color', '#D95319', 'LineWidth',1.4);
...
hold on
text(0.5, 2.5, '(e)', 'FontSize',10, 'FontWeight', 'bold')



subplot('position',[0.71 0.1 0.21 0.35])

[c,lags] = xcorr(KI(:,2:end), GOA_PC2(2:end), 5, 'normalized');
stem(lags,c, 'filled', 'LineWidth', 1.4);
ylim([-0.7 0.7]);
hold on
vline(0, '--', 'Color', '#bdbdbd');
text(-4, -0.55, 'KI leads', 'FontSize',10, 'FontWeight', 'bold')
text(1, -0.55, 'EOF leads', 'FontSize',10, 'FontWeight', 'bold')
text(-4.75, 0.6, '(f)', 'FontSize',10, 'FontWeight', 'bold')
set(gca,'YTickLabel',[]);
yyaxis right
ylim([-0.65 0.65]);
ax = gca;
...
%print('Nish_101322_KE_MHW_GOA_EOF_Fig','-dpng', '-r500');

%print('Nish_021423_KE_MHW_GOA_EOF_Fig','-dpng', '-r500');
print('Nish_051423_KE_MHW_GOA_EOF_Fig','-dpng', '-r500');
%% Explained variance
